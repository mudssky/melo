...
- [x] Update the old plan status note to reference this rebaseline plan
- [x] Write a failing integration test that initializes the database through the new `Migrator` and asserts the schema exists without calling any `rusqlite` bootstrap helper directly
- [x] Run that test and confirm it fails because the `SeaORM` migration path is missing
- [x] Add SeaORM entities and an initial migration matching the current schema
- [x] Replace `connect()` so production repositories receive a SeaORM database connection instead of a `rusqlite::Connection`
- [x] Convert library and playlist repositories to use SeaORM queries for create/list/update paths
- [x] Restrict `rusqlite` usage to tests and the standalone beets migration script only
- [x] Run `cargo test --test db_bootstrap -- --nocapture`
- [x] Run `cargo test --test seaorm_repository -- --nocapture`
...
- [x] Write a failing test for `LoftyMetadataReader` using a small tagged fixture file and assert title/lyrics/format extraction
- [x] Run the new `lofty_reader` test and confirm the failure comes from the missing concrete reader implementation
- [x] Implement `LoftyMetadataReader` and move sidecar/embedded precedence resolution into the scan service using the configured priority order
- [x] Use `mime_guess` when storing or returning artwork MIME values
- [x] Replace any remaining placeholder reader references in production code
- [x] Run `cargo test --test lofty_reader -- --nocapture`
- [x] Run `cargo test --test library_scan -- --nocapture`
...
- [x] Write a failing test that proves `PlayerService` can still be exercised against a fake backend while the production backend type becomes `RodioBackend`
- [x] Run the player test and verify the failure is caused by the missing real backend integration
- [x] Implement `RodioBackend` behind the existing `PlaybackBackend` trait
- [x] Update daemon app wiring so production state uses `RodioBackend`, while tests continue to use a fake or noop backend
- [x] Run `cargo test --test player_service -- --nocapture`
...
- [x] Write a failing test that checks the daemon router exposes the WebSocket route and that the TUI client can consume a pushed player snapshot
- [x] Run the API/TUI communication tests and verify the failure is due to the missing WebSocket path or client
- [x] Add `tower-http` trace middleware to the daemon router
- [x] Initialize `tracing-subscriber` in the binary entry point with environment-based filtering
- [x] Implement the daemon WebSocket endpoint and the TUI Rust client based on `tokio-tungstenite`
- [x] Run `cargo test --test api_server -- --nocapture`
- [x] Run `cargo test --test cli_remote -- --nocapture`
...
- [x] Write a failing TUI rendering-focused test that checks CJK titles do not break width calculations in list rows
- [x] Run the TUI tests and verify the failure is tied to missing width-aware rendering helpers
- [x] Introduce a real TUI layout and rendering layer
- [x] Use `unicode-width` in text measurement helpers for song titles and related columns
- [x] Keep the current keyboard-action behavior green while moving from state-only TUI code to UI-backed code
- [x] Run `cargo test --test tui_app -- --nocapture`
...
- [x] Write failing help-output tests for:
  - `melo --help`
  - `melo library --help`
  - `melo playlist --help`
  - `melo db --help`
- [x] Verify they fail because `about`, `long_about`, and example text are missing
- [x] Add descriptive `about` text for top-level and first-level commands
- [x] Add `long_about` text and examples for commands with tricky boundaries, especially `queue`, `playlist`, and `organize`
- [x] Run `cargo test --test cli_help -- --nocapture`
...
- [x] Update the old phase-1 plan so completed tasks remain checked and partially completed tasks reference this rebaseline plan
- [x] Review `Cargo.toml` and confirm that every phase-1-mandatory dependency is actually used in production code
- [x] Confirm that non-mandatory dependencies (`inquire`, `utoipa`) remain explicitly optional for later work
- [x] Run `cargo test --tests -- --nocapture`
- [x] Run `cargo check`
- [x] Run `python -m unittest scripts.tests.test_beets_to_melo -v`
- [x] Commit with `chore: complete phase 1 dependency convergence`
